...
const App = ({ handleAuthCallback, findTokenKey }) => {
  ...
  useEffect(() => {
    ...
    if (Capacitor.isNativePlatform()) {
      const handleDeepLink = (event) => {
        ...
        if (event.url.includes("ejtictactoedemo://callback")) {
  ...
  const initiateMobileLogin = async() => {
    try {
      const response = await fetch(`https://localhost:42123/pkce/MobileLoginUrl?returnUrl=http://localhost:3000/app/index.html`, { method: "GET" });
      if (!response.ok) {
    ...
    <div className="container">
      <nav className="navbar">
        <ul className="nav-list">
          ...
          <li
            className="nav-item"
            style={{ color: "white", marginTop: "1em", fontWeight: "bold" }}>ejtictactoedemo</li>
        </ul>
        {isLoggedIn ? (
